"""
Benchmarking script for ViT-SOM model
...
def main():
    ...
    for run in range(n_runs):
        ...
        logger = TensorBoardLogger('experiments/logs', name='vit_som', log_graph=True)
        lr_monitor = LearningRateMonitor(logging_interval='epoch')
        final_model = ModelCheckpoint(dirpath='experiments/states/vit_som', filename=f'vit_som_{dataset_name}_final', save_last=True)
        ...
        best_model = ViTSOM.load_from_checkpoint(os.path.join(final_model.dirpath, final_model.filename + '.ckpt'))
        ...
        for filename in os.listdir('experiments/states/vit_som'):
            file_path = os.path.join('experiments/states/vit_som', filename)
