...
def main():
    '''
    Main function to execute the training and evaluation pipeline for MobileViT.
    '''
    parser = argparse.ArgumentParser(description="Benchmarking script for MobileViT model")
    ...
    model_states_dir = f"experiments/states/mobile_vit/{dataset_name}"

    # benchmarking loop
    n_runs = 5
    all_metrics = {'accuracy': [], 'precision': [], 'recall': [], 'f1': [], 'run_duration': [], 'inference_duration': []}
    ...
    for run in range(n_runs):
        ...
        print('Training MobileViT model...')
        model = MobileViT(config=config)

        logger = TensorBoardLogger('experiments/logs', name=f'mobile_vit/{dataset_name}')
        ...
        best_model = MobileViT.load_from_checkpoint(checkpoint.best_model_path, config=config)
        ...
        all_metrics['inference_duration'].append(inference_time)
