...
def main():
    ...
    for run in range(n_runs):
        ...
        model = ViTClassifier(config=config)

        logger = TensorBoardLogger('experiments/logs', name=f'vit/{dataset_name}', log_graph=True)
        lr_monitor = LearningRateMonitor(logging_interval='epoch')

        model_checkpoint = ModelCheckpoint(monitor=f'val/accuracy', mode='max', dirpath=model_states_dir, filename=f'vit_{dataset_name}_best', save_top_k=1)
        ...
        best_model = ViTClassifier.load_from_checkpoint(model_checkpoint.best_model_path, config=config)
